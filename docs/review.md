...
| R3 | Important | `CLAUDE.md`, repository root | The file is committed and public. It carries the working instructions for this project — including the ssh alias and the *path* to the Hetzner token (not the token), and a line about "the single easiest way to ruin this submission" that reads oddly to anybody but its author | **Decided 30 July: it stays.** The author's call, and the reasoning is the same one the rest of this repository runs on. The file states rules — never invent a scoring threshold, stop and ask if the sanity table is missing — that the git history then shows being obeyed, so a reader gets the claim and the evidence side by side. "How much of this was written with AI assistance" is a question a 2026 reviewer will ask, and answering it in the open beats waiting to be asked. Removing the file the day before submission would delete evidence from a submission whose whole argument is that claims carry proof.

Two things did change. The credential path went in `69dfba5`. The Valis tooling block — 616 of the file's 1,182 words, none of it about weather — was removed in `7a84c0d`, silently rewritten back by a hook inside an unrelated commit (`4e4e52f`, a shutdown fix), and removed again in `425a6c8`. That regression is its own small finding: `git add -A` folded a doc edit into a code commit, which is exactly what constitution 11 forbids, in the file the rule is written in |
