...
| **The GraphQL schema is not a reviewable file** | Code-first means the SDL exists only at runtime. Named as a cost in [ADR 0002](./adr/0002-yoga-pothos-code-first.md) | A printed-SDL snapshot test, roughly ten lines |
| **No CI** | Tests and typecheck run locally and before every commit; nothing enforces it on push | A GitHub Actions workflow running `tsc --noEmit` and `pnpm test` |
