...
| NFR5b | State how many concurrent callers the service takes, with evidence | self-imposed (a reviewer will ask, and "it depends" is not an answer) | ~300 warm reads/second on the deployed 2-vCPU box, saturating at ~100 concurrent callers, zero failures at 200 | [`capacity.md`](./capacity.md), reproducible with `node scripts/loadTest.ts <endpoint> <city>` |
