...
| NFR9 | No scoring number whose provenance a reader cannot check | self-imposed (constitution) | 17 of the 18 factor and gate sources carry a publication; the one that does not says `NOT CITED` and gives its reasoning, and the one fitted anchor (skiing's lift-hold gate at 72) says so in the source string and in [`sanity-table.md`](./sanity-table.md). Curves fitted to the table | All 23 sanity rows pass, and four tests require every source to carry a link or say `NOT CITED` — the old assertion was `length > 20`, which prose passes |
