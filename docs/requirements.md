...
| NFR6 | **Nothing inside the lease may outlive it** — not only the upstream call | self-imposed (risk 8, [recon](./recon.md), and it stated only half of this) | 8 s `AbortSignal` into `fetch`, plus `timeoutMS` on the MongoClient, against a 30 s lease ([ADR 0001](./adr/0001-mongodb-over-postgres.md)). Worst case 8 s + 4 database calls x 5 s = 28 s | Provider tests assert the signal reaches `fetch`; `client.test.ts` asserts the whole budget fits inside `LEASE_TTL_MS` |
