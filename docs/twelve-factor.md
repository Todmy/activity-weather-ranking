...
| IX | Disposability | **holds, fixed 30 July** | Boots in under a second. SIGTERM and SIGINT close the socket, drop idle keep-alive connections immediately, give a request already in flight twelve seconds, then force. The refresher's tick is awaited before the database handle closes, so no lease is stranded |
